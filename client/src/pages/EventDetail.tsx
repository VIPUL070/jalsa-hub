...
const EventDetail = () => {
  ...
  useEffect(() => {
    const fetchEvent = async () => {
      const { data } = await api.get<ApiResponse>(`/events/${id}`);
      try {
        setEvent(data.userEvents);
      } catch {
            ...
            <button
              ...
              className={`w-full py-4 px-6 rounded-xl font-bold text-lg transition shadow-lg ${
                isSoldOut || (successMsg && !showOTP)
                  ...
                  : "bg-gray-900 hover:bg-black text-white cursor-pointer hover:shadow-xl hover:-translate-y-1"
              }`}
            >
              {bookingLoading
                ? "Processing..."
